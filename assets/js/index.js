...
const alertModal = {
	init : function (title, bodyText){
		this.title = title;
		this.bodyText = bodyText;
		if (document.body.appendChild(this.modalBody())) {
			this.modal = new bootstrap.Modal(this.modalBody(), {keyboard: false});
			this.modal.show();
			this.reloadAlert();
		}
	},
	...
	modalBody : function(){
		...
		modalContainer.setAttribute('aria-labelledby', 'alertModal');
		modalContainer.setAttribute('aria-hidden', 'true');
		modalContainer.id = "alertModal";
		...
		modalTitle.id = 'alertModalTitle';
		modalTitle.innerHTML = this.title;
		/* add modal-title to modal-header */
		...
		/* add btn-close to modal-header */
		modalHeader.appendChild(closeBtn);
		/* add modal-header to modal-content */
		modalContent.appendChild(modalHeader);

		/* modal-body */
		let modalBody = document.createElement('div');
		modalBody.className = 'modal-body';
		modalBody.innerHTML = this.bodyText;
		...
		cancelBtn.setAttribute('class', 'btn btn-sm btn-primary');
		cancelBtn.setAttribute('data-bs-dismiss', 'modal');
		cancelBtn.innerText = 'OK';
		/* add dismiss button to modal-footer */
		modalFooter.appendChild(cancelBtn);
		/* add modal-footer to modal-content */
	...
	reloadAlert: () => {
		...
		for (let qp of queryParams.entries()) {
			if (qp[0]!='action' && qp[0]!='action_msg') {
				params[qp[0]] = qp[1];
			}
		}
		for (let qs in params) {
		...
		return window.history.pushState('','', url!='' ? url : window.location.origin + window.location.pathname);
	},
}
/*-------------------------------{ Custom alert modal ends }-------------------------------*/
