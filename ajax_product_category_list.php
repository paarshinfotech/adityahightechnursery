...
<?php
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM category_product WHERE product_cat_status='1' " . $SQL_Filter_Query . " ;";
...
<table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
    <thead>
        <tr>
            <th>
                ...
                <div class="d-inline-flex align-items-center select-all">
                    <input type="checkbox" class="multi-check-all mt-0 form-check-input fs-6"
                        id="delete_check_box_Product_Category" onchange="checkAll(this , 'multi-check-item')">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="category-action">
                            <li>
                                <a title="प्रॉडक्ट श्रेणी हटवा" class="multi-action-btn dropdown-item text-danger"
                                    data-multi-action="delete" data-multi-action-page="" href="#"
                                    onclick="performDelete('PRODUCTCATEGORY', 1, '#delete_check_box', ajaxProductCategoryData)">
    ...
    <tbody>
        <?php $getBrand = mysqli_query($connect, "SELECT * FROM category_product WHERE product_cat_status='1' " . $SQL_Filter_Query . " ORDER BY cat_id DESC " . $SQL_ROW_Query . ";") ?>
        ...
        <tr>
            <td class="form-group">
                <input type="checkbox" class="multi-check-item  multi_check_Item" id="delete_check_box" name="cat_id[]"
                    value="<?= $cat_id ?>">
            </td>
            <td>
