...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (true) { ?>
        ...
        if (!empty($_POST['Search'])) {
            ...
            $SQL_Filter_Query .= "AND  (name LIKE '%$searchTerm%' OR mobile LIKE '%$searchTerm%' OR mob_id LIKE '%$searchTerm%') ";
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM mobile_diary WHERE status='1' " . $SQL_Filter_Query . " ;";
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="mobtbl" class="table table-striped table-bordered table-hover multicheck-container">
                <thead>
                    <tr>
                        <th>
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="Delete" class="multi-action-btn dropdown-item text-danger"
                                                ...
                                                onclick="performDelete('MOBILEDARY', 1, '#delete_check_box', ajaxMobileDiarylist)">
                ...
                <tbody>
                    <?php $getMobDiary = mysqli_query($connect, "SELECT * FROM mobile_diary WHERE status='1' ".$SQL_Filter_Query." ORDER BY mob_id DESC ".$SQL_ROW_Query." ;") ?>
                    <?php if (mysqli_num_rows($getMobDiary) > 0): ?>
                        <?php while ($mobDiary = mysqli_fetch_assoc($getMobDiary)):
                            ...
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" id="delete_check_box" name="mob_id[]" value="<?= $mob_id ?>">
