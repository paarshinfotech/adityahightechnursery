...
if (isset($_GET['restore']) && isset($_GET['ex_cat_id'])){
    escapePOST($_GET);
    ...
    // }
     $delete = mysqli_query($connect, "UPDATE expenses_category SET ex_cat_status='1' WHERE ex_cat_id='{$dir}'");
    }
        if($delete){
    	header("Location: expenses_category_list?action=Success&action_msg=खर्च श्रेणी पुनर्संचयित कले..!");
		exit();
        }else{
...
if (isset($_GET['delete']) && isset($_GET['ex_cat_id'])){
    ...
    //del profile and driver 
    foreach ($_GET['ex_cat_id'] as $dir){
    $delete = mysqli_query($connect, "DELETE FROM expenses_category WHERE ex_cat_id='{$dir}'");
     }
        if($delete){
    	header("Location: expenses_category_list?action=Success&action_msg=खर्च श्रेणी् हटवले..!");
		exit();
        }else{
        header('Location: expenses_category_list?action=Success&action_msg=काहीतरी चूक झाली');
      	exit();
...
<div class="page-wrapper">
	<div class="page-content">
		<h6 class="mb-0 text-uppercase">खर्च श्रेणी </h6>
            <a class="btn btn-sm btn-success float-end" href="expenses_category_add" style="margin-top:-25px;">
						...
						<thead>
							<tr>
								<th>
			                            ...
			                            <div class="dropdown">
			                            	...
			                            	<ul class="dropdown-menu" aria-labelledby="category-action">
			                            		...
			                            		<li>
			                            			...
			                            			<li>
			                            			<a title="खर्च श्रेणी हटवा" class="multi-action-btn dropdown-item text-danger" data-multi-action="delete" data-multi-action-page="" href="?delete=true" onclick="return confirm('खर्च श्रेणी हटवा..?');">
			                            				<i class="btn-round bx bx-trash me-2"></i>खर्च श्रेणी हटवा 
			                            			</a>
			                            		</li>
			                            	</ul>
			                            </div>
    								...
    								<th>आयडी</th>
                                    <th>खर्च श्रेणीचे नाव</th
                                    >
						   </tr>
						</thead>
						<tbody>
                            <?php $getBrand = mysqli_query($connect, "SELECT * FROM expenses_category WHERE ex_cat_status='0' ORDER BY ex_cat_id DESC") ?>
                            <?php if (mysqli_num_rows($getBrand)>0): ?>
                                <?php while ($brand = mysqli_fetch_assoc($getBrand)): 
                                extract($brand);
                                
                                $gettotal=mysqli_query($connect,"SELECT SUM(ex_amt) as totexpense FROM expenses where ex_cat_id='{$ex_cat_id}'");
                                        $totaltotexpense=mysqli_fetch_assoc($gettotal);
                                ?>
                                <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" name="ex_cat_id[]" value="<?= $ex_cat_id ?>">
                                </td>
                                <td>
                                ...
                                <td>
                                    <a class="text-decoration-none" href="expenses_category_add?ex_cat_id=<?= $ex_cat_id;?>">
                                        <?= $ex_cat_name; ?>
                                    </a>
                                </td>
                                <td>₹ <?= $totaltotexpense['totexpense'] ?>/-</td>
                            </tr>
                                <?php endwhile ?>
                            <?php endif ?>
						</tbody>
