...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (true) { ?>
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM jcb_category WHERE jcb_cat_status='1' " . $SQL_Filter_Query . " ;";
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
                            ...
                            <div class="d-inline-flex align-items-center select-all">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="जेसीबी श्रेणी हटवा" class="multi-action-btn dropdown-item text-danger"
                                                data-multi-action="delete" data-multi-action-page="" href="#"
                                                onclick="performDelete('JCBCATEGORY', 1, '#delete_check_box', ajaxJCBCatgory)">
                ...
                <tbody>
                    <?php $getcat = mysqli_query($connect, "SELECT * FROM jcb_category WHERE jcb_cat_status='1' " . $SQL_Filter_Query . " ORDER BY jcb_cat_id DESC " . $SQL_ROW_Query . " ;") ?>
                    <?php if (mysqli_num_rows($getcat) > 0): ?>
                        <?php while ($cat = mysqli_fetch_assoc($getcat)):
                            extract($cat);
                            $gettotal = mysqli_query($connect, "SELECT SUM(trip) as tottrip FROM jcb where jcb_cat_id='$jcb_cat_id' AND jcb_status!=0");
                            $ttrip = mysqli_fetch_assoc($gettotal);
                            ?>
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" id="delete_check_box" name="jcb_cat_id[]"
                                        value="<?= $jcb_cat_id ?>">
                                ...
                                <td>
                                    <a class="text-decoration-none" href="jcb_list?jcb_cat_id=<?= $jcb_cat_id ?>">
                                        <?= $cat_name; ?>
                                    </a>
                                </td>
                                <?php if ($ttrip['tottrip'] == 0) { ?>
                                    <td>₹ 0/-</td>
                                <?php } else { ?>
                                    <td>₹
                                        <?= $ttrip['tottrip'] ?>/-
