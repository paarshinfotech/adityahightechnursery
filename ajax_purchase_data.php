...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (true) { ?>
        ...
        //     $SQL_Filter_Query .= "AND purchase_name LIKE '%" . mysqli_real_escape_string($connect, $_POST['Search']) . "%'";
        // }
        if (!empty($_POST['Search'])) {
            $searchTerm = mysqli_real_escape_string($connect, $_POST['Search']);
            $SQL_Filter_Query .= "AND  (purchase_name LIKE '%$searchTerm%' OR purchase_id LIKE '%$searchTerm%' ) ";
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM purchase WHERE purchase_status='1' " . $SQL_Filter_Query . " ;";
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="purchasetbl" class="table table-striped table-bordered table-hover multicheck-container">
                <thead>
                    <tr>
                        <th>
                            <?php //if ($auth_permissions->brand->can_delete): ?>
                            <div class="d-inline-flex align-items-center select-all">
                                <input type="checkbox" onchange="checkAll(this , 'multi_check_Purchase')" class="multi-check-all mt-0 form-check-input fs-6">
                                <div class="dropdown">
                                    <button class="btn btn-sm dropdown" type="button" id="category-action" data-bs-toggle="dropdown"
                                        aria-expanded="false">
                                        <i class='bx bx-slider-alt fs-6'></i>
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="खरेदी हटवा" class="multi-action-btn dropdown-item text-danger"
                                                data-multi-action="delete" data-multi-action-page="" href="#"
                                                onclick="performDelete('PURCHASE', page = 1, '#delete_check_box_Purchase' , ajaxPurchaseData)">
                ...
                <tbody>
                    <?php $getPurchase = mysqli_query($connect, "SELECT * FROM purchase WHERE purchase_status='1'  " . $SQL_Filter_Query . " ORDER BY purchase_id DESC " . $SQL_ROW_Query . ";");
                    ?>
                    <?php if (mysqli_num_rows($getPurchase) > 0): ?>
                        <?php while ($purchase = mysqli_fetch_assoc($getPurchase)):
                            ...
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox"  id="delete_check_box_Purchase"  class="multi-check-item  multi_check_Purchase" name="purchase_id[]" value="<?= $purchase_id ?>">
                                    <span class="badge bg-gradient-bloody text-white shadow-sm">
                                        <?= $purchase_id ?>
                                    </span>
                                </td>
                                <td>
                                    <a class="text-decoration-none" href="sales?show_purchase=true&pur_id=<?= $purchase_id; ?>">
                                        <?= $purchase_name ?>
                                    </a>
                                </td>
                                <td>
                ...
                <div class="col-sm-12 col-md-5 d-flex gap-2 align-items-center ">
                    ...
                    <div>
                        <input type="number" min="1" max="<?php echo $pages ?>" id="input_chang_page_customer"
                            value="<?php echo $page ?>" onchange="ChangePagePurchase(page = this.value);"
                    ...
                    <div class="dataTables_paginate paging_simple_numbers" id="salestbl-filter_paginate">
                        <ul class="pagination">
                            ...
                            if ($onpage > 1) {
                                ...
                                echo '<a href="#" onclick="ChangePagePurchase(' . ($onpage - 1) . ')" class="page-link">Prev</a>';
                                echo '</li>';
                            }
                            ...
                            if ($onpage > 3) {
                                ...
                                echo '<a href="#" onclick="ChangePagePurchase(1)" class="page-link">1</a>';
                                echo '</li>';
                            ...
                            for ($i = max(1, $onpage - 1); $i <= min($pages, $onpage + 1); $i++) {
                                if ($i == $onpage) {
                                    ...
                                    echo '<li class="paginate_button page-item">';
                                    echo '<a href="#" onclick="ChangePagePurchase(' . $i . ')" class="page-link">' . $i . '</a>';
                                    echo '</li>';
                                }
                            }
                            ...
                            if ($onpage < $pages - 1) {
                                ...
                                echo '<li class="paginate_button page-item">';
                                echo '<a href="#" onclick="ChangePagePurchase(' . $pages . ')" class="page-link">' . $pages . '</a>';
                                echo '</li>';
                            }
                            ...
                            if ($onpage < $pages) {
                                ...
                                echo '<a href="#" onclick="ChangePagePurchase(' . ($onpage + 1) . ')" class="page-link">Next</a>';
                                echo '</li>';
                            }
                            ?>
                        </ul>
