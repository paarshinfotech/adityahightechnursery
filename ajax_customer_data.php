        ...
        <ul class="dropdown-menu" aria-labelledby="category-action">
            <li>
                <a title="Delete" class="multi-action-btn dropdown-item text-danger" data-multi-action="delete"
                    data-multi-action-page="" href="#" onclick="return confirm('ग्राहक हटवा..?');">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (true) { ?>
        ...
        if (!empty($_POST['Search'])) {
            ...
            $SQL_Filter_Query .= "AND  (customer_name LIKE '%$searchTerm%' OR customer_id LIKE '%$searchTerm%' OR customer_mobno LIKE '%$searchTerm%') ";
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM customer WHERE customer_status='1' " . $SQL_Filter_Query . " ;";
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="customertbl" class="table table-striped table-bordered table-hover multicheck-container">
                <thead>
                    <tr>
                        <th>
                            <?php //if ($auth_permissions->brand->can_delete): ?>
                            <div class="d-inline-flex align-items-center select-all">
                                <input type="checkbox" onchange="checkAll(this , 'multi_check_Customer_data')"
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="Delete" class="multi-action-btn dropdown-item text-danger"
                                                ...
                                                onclick="performDelete('CUSTOMER', page = 1, '#delete_check_box_Custtome' , ajaxCustomerData)">
                ...
                <tbody>
                    <?php $getBrand = mysqli_query($connect, "SELECT * FROM customer WHERE customer_status='1' " . $SQL_Filter_Query . " ORDER BY customer_id DESC " . $SQL_ROW_Query . ";") ?>
                    <?php if (mysqli_num_rows($getBrand) > 0): ?>
                        <?php while ($brand = mysqli_fetch_assoc($getBrand)):
                            ...
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" id="delete_check_box_Custtome" class="multi-check-item multi_check_Customer_data"
                ...
                <div class="col-sm-12 col-md-5 d-flex gap-2 align-items-center ">
                    ...
                    <div>
                        <input type="number" min="1" max="<?php echo $pages ?>" id="input_chang_page_customer"
                            value="<?php echo $page ?>" onchange="ChangePageCustomer(page = this.value);"
                    ...
                    <div class="dataTables_paginate paging_simple_numbers" id="salestbl-filter_paginate">
                        <ul class="pagination">
                            ...
                            if ($onpage > 1) {
                                ...
                                echo '<a href="#" onclick="ChangePageCustomer(' . ($onpage - 1) . ')" class="page-link">Prev</a>';
                                echo '</li>';
                            }
                            ...
                            if ($onpage > 3) {
                                ...
                                echo '<a href="#" onclick="ChangePageCustomer(1)" class="page-link">1</a>';
                                echo '</li>';
                            ...
                            for ($i = max(1, $onpage - 1); $i <= min($pages, $onpage + 1); $i++) {
                                if ($i == $onpage) {
                                    ...
                                    echo '<li class="paginate_button page-item">';
                                    echo '<a href="#" onclick="ChangePageCustomer(' . $i . ')" class="page-link">' . $i . '</a>';
                                    echo '</li>';
                                }
                            }
                            ...
                            if ($onpage < $pages - 1) {
                                ...
                                echo '<li class="paginate_button page-item">';
                                echo '<a href="#" onclick="ChangePageCustomer(' . $pages . ')" class="page-link">' . $pages . '</a>';
                                echo '</li>';
                            }
                            ...
                            if ($onpage < $pages) {
                                ...
                                echo '<a href="#" onclick="ChangePageCustomer(' . ($onpage + 1) . ')" class="page-link">Next</a>';
                                echo '</li>';
                            }
                            ?>
                        </ul>
