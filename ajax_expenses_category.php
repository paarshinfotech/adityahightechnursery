...
<?php
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM expenses_category WHERE ex_cat_status='1' " . $SQL_Filter_Query . " ;";
...
<table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
                ...
                <div class="d-inline-flex align-items-center select-all">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="category-action">
                            <li>
                                <a title="खर्च  श्रेणी हटवा" class="multi-action-btn dropdown-item text-danger"
                                    data-multi-action="delete" data-multi-action-page="" href="#"
                                    onclick="performDelete('EXPENSESCATEGORY', 1, '#delete_check_box', ajaxExpensesCategory)">
    ...
    <tbody>
        <!--SELECT * FROM expenses_category c,expenses e WHERE c.ex_cat_status='1' and e.ex_cat_id=c.ex_cat_id group BY c.ex_cat_id DESC-->
        <?php $getBrand = mysqli_query($connect, "SELECT * FROM expenses_category WHERE ex_cat_status='1' " . $SQL_Filter_Query . " ORDER BY ex_cat_id DESC " . $SQL_ROW_Query . " ;")
                        ?>
        <?php if (mysqli_num_rows($getBrand) > 0): ?>
        <?php while ($brand = mysqli_fetch_assoc($getBrand)):
                            extract($brand);

                            $gettotal = mysqli_query($connect, "SELECT SUM(ex_amt) as totexpense FROM expenses where ex_cat_id='{$ex_cat_id}' and ex_status='1'");
                            $totaltotexpense = mysqli_fetch_assoc($gettotal);
                            ?>
        <tr>
            <td class="form-group">
                <input type="checkbox" class="multi-check-item" id="delete_check_box" name="ex_cat_id[]"
                    value="<?= $ex_cat_id ?>">
            ...
            <td>
                <a class="text-decoration-none" href="expenses?ex_cat_id=<?= $ex_cat_id; ?>">
                    <?= $ex_cat_name; ?>
                </a>
            </td>
        ...
        <?php error_reporting(0);
                            $totExpenseCat += $totaltotexpense['totexpense'] ?>
        <?php endwhile ?>
        <?php endif ?>
    </tbody>
    <?php error_reporting(0); ?>
