...
//restore 
if (isset($_GET['restore']) && isset($_GET['bank_inward_id'])){
    escapePOST($_GET);
   
    //del profile and driver 
    foreach ($_GET['bank_inward_id'] as $dir){
     $delete = mysqli_query($connect, "UPDATE bank_inward SET bank_inward_status='1' WHERE inward_id='{$dir}'");
    }
        if($delete){
    	header("Location: oneday_add?show_income=true&action=Success&action_msg=बँक आवक पुनर्संचयित  केले..!");
		exit();
        }else{
...
if (isset($_GET['delete']) && isset($_GET['bank_inward_id'])){
    ...
    //del profile and driver 
    foreach ($_GET['bank_inward_id'] as $dir){
    $delete = mysqli_query($connect, "DELETE FROM bank_inward WHERE inward_id='{$dir}'");
     }
    //  $delete = mysqli_query($connect, "UPDATE customer SET bank_inward_status='1' WHERE inward_id='{$dir}'");
    // }
        if($delete){
...
<div class="page-wrapper">
	<div class="page-content">
		<h6 class="mb-0 text-uppercase">बँक आवक</h6>
				 ...
				 <div class="table-responsive">
					<table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
						<thead>
			                            ...
			                            <div class="dropdown">
			                            	...
			                            	<ul class="dropdown-menu" aria-labelledby="category-action" style="margin: 0px; position: absolute; inset: 0px auto auto 0px; transform: translate(0px, 35px);">
			                            		<li>
			                            			...
			                            			<li>
			                            			<a title="बँक आवक हटवा" class="multi-action-btn dropdown-item text-danger" data-multi-action="delete" data-multi-action-page="" href="?delete=true" onclick="return confirm('बँक आवक हटवा..?');">
			                            				<i class="btn-round bx bx-trash me-2"></i>बँक आवक हटवा 
			                            			</a>
			                            		</li>
			                            	</ul>
							...
							<tbody>
                                <?php $getBrand = mysqli_query($connect, "select * from bank_inward WHERE bank_inward_status='0' ORDER BY inward_id DESC") ?>
                                <?php if (mysqli_num_rows($getBrand)>0): ?>
                                    ...
                                    <tr>
                                    <td class="form-group">
                                        <input type="checkbox" class="multi-check-item" name="bank_inward_id[]" value="<?= $inward_id ?>">
                                    ...
                                    <td><a class="text-decoration-none" href="?show_income=true&bank_inward_id=<?= $inward_id;?>"><?= $inward?></a></td>
                                    <td>₹ <?= $inward_rs?>/-</td>
                                </tr>
                                    <?php endwhile ?>
                                <?php endif ?>
    						</tbody>
    						<?php $gettotal=mysqli_query($connect,"SELECT SUM(inward_rs) as totbankinward FROM bank_inward WHERE bank_inward_status='0'");
                                        $total_bank_inward=mysqli_fetch_assoc($gettotal);
                                        ?>
                                            <tr>
                                                <th colspan="3" class="text-center fs-6">एकूण</th>
                                                <th class="text-nowrap">₹ <?= $total_bank_inward['totbankinward'] ?>/-</th>
