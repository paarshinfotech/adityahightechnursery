        ...
        <ul class="dropdown-menu" aria-labelledby="category-action">
            <li>
                <a title="Delete" class="multi-action-btn dropdown-item text-danger" data-multi-action="delete"
                    data-multi-action-page="" href="?delete=true" onclick="return confirm('ग्राहक हटवा..?');">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (true) { ?>
        ...
        <?php
        $SQL_Filter_Query = '';
        // $SQL_Order_Query = 'ORDER BY customer_id DESC';
        ...
        if (!empty($_POST['city'])) {
            if (!empty($_POST['taluka'])) {
                if (!empty($_POST['village'])) {
                    ...
                    $SQL_Filter_Query .= "AND taluka = '" . mysqli_real_escape_string($connect, $_POST['taluka']) . "'";
                }
            } else {
                $SQL_Filter_Query .= "AND city = '" . mysqli_real_escape_string($connect, $_POST['city']) . "'";
            }
        }
        // if (!empty($_POST['village'])) {
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="custbl" class="table table-striped table-bordered table-hover multicheck-container">
                <thead>
                    <tr>
                        <th>
                            <?php //if ($auth_permissions->brand->can_delete): ?>
                            <div class="d-inline-flex align-items-center select-all">
                                <input type="checkbox" class="multi-check-all mt-0 form-check-input fs-6"
                                    id="delete_check_box_Product" onchange="checkAll(this , 'multi_check_Custome')">
                                <div class="dropdown">
                                    <button class="btn btn-sm dropdown" type="button" id="category-action" data-bs-toggle="dropdown"
                                        aria-expanded="false">
                                        <i class='bx bx-slider-alt fs-6'></i>
                ...
                <tbody>
                    ...
                    <?php if (mysqli_num_rows($getBrand) > 0): ?>
                        <?php while ($brand = mysqli_fetch_assoc($getBrand)):
                            extract($brand);
                            // print $brand ;
                            // echo "id = ". $brand["customer_id"] ."<br>" ;
                            ?>
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" id="delete_check_box_Custtome" class="multi-check-item multi_check_Custome"
