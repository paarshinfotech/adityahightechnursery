...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (true) { ?>
        ...
        if (!empty($_POST['Search'])) {
            $SQL_Filter_Query .= "AND customer_name LIKE '%" . mysqli_real_escape_string($connect, $_POST['Search']) . "%'";
        }
        // if (!empty($_POST['city'])) {
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM letter_pad WHERE letter_status='1' " . $SQL_Filter_Query . " ;";
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="custbl" class="table table-striped table-bordered table-hover multicheck-container">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="Delete" class="multi-action-btn dropdown-item text-danger"
                                                ...
                                                onclick="performDelete('LETTERLIST', 1, '#delete_check_box', ajaxLetterData)">
                ...
                <tbody>
                    <?php $getBrand = mysqli_query($connect, "SELECT * FROM letter_pad WHERE letter_status='1' ". $SQL_Filter_Query ." ORDER BY lid DESC " . $SQL_ROW_Query . ";") ?>
                    <?php if (mysqli_num_rows($getBrand) > 0): ?>
                        <?php while ($brand = mysqli_fetch_assoc($getBrand)):
                            ...
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" id="delete_check_box" name="lid[]" value="<?= $lid ?>">
                                    <span class="badge bg-gradient-bloody text-white shadow-sm">
                                        <?= $lid ?>
                                        <? //=  html_entity_decode($letters)?>
                                    </span>
                                </td>
                                ...
                                <td class="text-nowrap">
                                    <a href="letter_invoice?lid=<?php echo $lid ?>" class="text-inverse p-r-10" data-bs-toggle="tooltip"
                                        title="Invoice" data-original-title="Invoice"><i class="fa fa-print text-cyan"></i></a>
                                    <a href="letter_pad?lid=<?php echo $lid ?>" class="text-inverse p-r-10" data-bs-toggle="tooltip" title=""
