                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="गाडी भाडे श्रेणी हटवा" class="multi-action-btn dropdown-item text-danger"
                                                ...
                                                onclick="performDelete('CARRENTALCATEGORY', 1, '#delete_check_box', ajaxCarrentalCategoryData)">
                ...
                <tbody>
                    ...
                    <?php if (mysqli_num_rows($getBrand) > 0): ?>
                        <?php while ($brand = mysqli_fetch_assoc($getBrand)):
                            ...
                            $getdep = mysqli_query($connect, "SELECT SUM(deposit_rs) as totdeposit_rs FROM car_rental WHERE car_cat_id='$car_cat_id' and car_status='1'");
                            $totdep = mysqli_fetch_assoc($getdep);
                            ?>
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" id="delete_check_box" name="car_cat_id[]"
                                        value="<?= $car_cat_id ?>">
                            ...
                            $totCarRentalAmt += $totdep['totdeposit_rs'] ?>
                        <?php endwhile ?>
                    <?php endif ?>
                </tbody>
                <?php error_reporting(0); ?>
