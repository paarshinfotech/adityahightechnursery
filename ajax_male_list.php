...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (!empty($_POST['Gender'])) { ?>
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM employees WHERE emp_status='1' and emp_gender='$Gender' " . $SQL_Filter_Query . " ;";
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="Delete" class="multi-action-btn dropdown-item text-danger"
                                                ...
                                                onclick="performDelete('EMPLOYEE', 1, '#delete_check_box', ajaxMaleEmpolyee)">
                ...
                <tbody>
                    <?php $getEmp = mysqli_query($connect, "SELECT * FROM employees WHERE emp_status='1' and emp_gender='$Gender' " . $SQL_Filter_Query . " ORDER BY emp_id DESC " . $SQL_ROW_Query . " ;") ?>
                    <?php if (mysqli_num_rows($getEmp) > 0): ?>
                        <?php while ($emp = mysqli_fetch_assoc($getEmp)):
                            ...
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" id="delete_check_box" name="emp_id[]"
                                        value="<?= $emp_id ?>">
