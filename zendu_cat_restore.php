...
if (isset($_GET['restore']) && isset($_GET['cat_id'])){
    ...
    // }
     $delete = mysqli_query($connect, "UPDATE marigold_category SET cat_status='1' WHERE cat_id='{$dir}'");
    }
        if($delete){
    	header("Location: zendu_cat_list?action=Success&action_msg=झेंडू  श्रेणी पुनर्संचयित कले..!");
		exit();
        }else{
...
if (isset($_GET['delete']) && isset($_GET['cat_id'])){
    ...
    //del profile and driver 
    foreach ($_GET['cat_id'] as $dir){
    $delete = mysqli_query($connect, "DELETE FROM marigold_category WHERE cat_id='{$dir}'");
     }
        if($delete){
    	header("Location: zendu_cat_list?action=Success&action_msg=झेंडू  श्रेणी् हटवले..!");
		exit();
        }else{
        header('Location: zendu_cat_list?action=Success&action_msg=काहीतरी चूक झाली');
      	exit();
			...
			<div class="card-body">
				 ...
				 <div class="table-responsive">
					<table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
			                            ...
			                            <div class="dropdown">
			                            	...
			                            	<ul class="dropdown-menu" aria-labelledby="category-action">
			                            		...
			                            		<li>
			                            			...
			                            			<li>
			                            			<a title="झेंडू  श्रेणी हटवा" class="multi-action-btn dropdown-item text-danger" data-multi-action="delete" data-multi-action-page="" href="?delete=true" onclick="return confirm('झेंडू  श्रेणी हटवा..?');">
			                            				<i class="btn-round bx bx-trash me-2"></i>झेंडू  श्रेणी हटवा 
			                            			</a>
			                            		</li>
			                            	</ul>
			                            </div>
						...
						<tbody>
                            <?php $getBrand = mysqli_query($connect, "SELECT * FROM marigold_category c, plant_type p WHERE cat_status='0' and p.pt_id=c.pt_id ORDER BY cat_id DESC") ?>
                            <?php if (mysqli_num_rows($getBrand)>0): ?>
                                ...
                                <td>
                                    <a class="text-decoration-none" href="zendu_cat_add?cat_id=<?= $cat_id;?>">
                                        <?= $cat_name; ?>
                                    </a>
                                </td>
                                 <td><?= $cat_qty?></td>
                                                
                                <td><?= date('d M Y',strtotime($cat_date))?></td> 
