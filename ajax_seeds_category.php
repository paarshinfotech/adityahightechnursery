...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (true) { ?>
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM seeds_category WHERE status='1' " . $SQL_Filter_Query . " ;";
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
                            ...
                            <div class="d-inline-flex align-items-center select-all">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="बियाणे आवक व जावक श्रेणी हटवा"
                                                class="multi-action-btn dropdown-item text-danger" data-multi-action="delete"
                                                data-multi-action-page="" href="#"
                                                onclick="performDelete('SEEDCATEGORY', 1, '#delete_check_box', ajaxSeedCategory)">
                ...
                <tbody>
                    <?php $getBrand = mysqli_query($connect, "SELECT * FROM seeds_category WHERE status='1' " . $SQL_Filter_Query . " ORDER BY cat_id DESC " . $SQL_ROW_Query . " ;") ?>
                    <?php if (mysqli_num_rows($getBrand) > 0): ?>
                        <?php while ($brand = mysqli_fetch_assoc($getBrand)):
                            extract($brand);

                            $getdep = mysqli_query($connect, "SELECT SUM(total) as totAmt FROM seeds_sales WHERE cat_id='{$cat_id}' and seeds_status='1'");
                            $totdep = mysqli_fetch_assoc($getdep);
                            ?>
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" id="delete_check_box" name="cat_id[]" value="<?= $cat_id ?>">
                                ...
                                <td>
                                    <a class="text-decoration-none" href="seeds_list?cat_id=<?= $cat_id; ?>">
                                        <?= $cat_name; ?>
                                    </a>
                                </td>
                                <td>₹
