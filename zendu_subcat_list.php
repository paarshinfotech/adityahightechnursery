...
if (isset($_GET['delete']) && isset($_GET['subcat_id'])) {
    ...
    foreach ($_GET['subcat_id'] as $dir) {
        ...
        // }
        $delete = mysqli_query($connect, "UPDATE marigold_subcategory SET sub_cat_status='0' WHERE subcat_id='{$dir}'");
    }
    if ($delete) {
        header("Location: zendu_subcat_list?action=Success&action_msg=झेंडू उपवर्ग श्रेणी हटवले..!");
        exit();
    } else {
                        ...
                        <div class="dataTables_length" id="suppliertbl_length">
                            <label style="display: inline-flex; align-items: center; gap: 5px; ">Show
                                <select name="suppliertbl_length" id="table_Row_Limit"
                                    onchange="ajaxZanduSubCategoryData(1)" aria-controls="suppliertbl"
...
<script>
    function ajaxZanduSubCategoryData(page = 1, search = '') {
...
function ChangePage(page){
	...
	ajaxZanduSubCategoryData(page, inputValue )
}
</script>
...
<script>
    function logInputValueCustomer() {
        ...
        ajaxZanduSubCategoryData(1, inputValue);
    }
</script>
