...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (true) { ?>
        ...
        //     $SQL_Filter_Query .= "AND emp_name LIKE '%" . mysqli_real_escape_string($connect, $_POST['Search']) . "%'";
        // }

        if (!empty($_POST['Search'])) {
            $searchTerm = mysqli_real_escape_string($connect, $_POST['Search']);
            $SQL_Filter_Query .= " AND  (emp_name LIKE '%$searchTerm%' OR emp_mobile LIKE '%$searchTerm%' OR emp_id LIKE '%$searchTerm%') ";
        ...
        if (!empty($_POST['Joining'])) {
            $SQL_Filter_Query .= "AND emp_joined = '" . $_POST['Joining'] . "'";
        }
        if (!empty($_POST['Name'])) {
            $SQL_Filter_Query .= "AND emp_name = '" . $_POST['Name'] . "'";
        }
        if (!empty($_POST['Gender'])) {
            $SQL_Filter_Query .= "AND emp_gender = '" . $_POST['Gender'] . "'";
        }
        ...
        $sql = "SELECT COUNT(*) AS total_rows FROM employees WHERE emp_status='1' " . $SQL_Filter_Query . " ;";
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="emptbl" class="table table-striped table-bordered table-hover multicheck-container">
                <thead>
                    <tr>
                        <th>
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="Delete" class="multi-action-btn dropdown-item text-danger"
                                                ...
                                                onclick="performDelete('EMPLOYEE', 1, '#delete_check_box', ajaxEmployee)">
                ...
                <tbody>
                    <?php $getEmp = mysqli_query($connect, "SELECT * FROM employees WHERE emp_status='1' ".$SQL_Filter_Query." ORDER BY emp_id DESC ".$SQL_ROW_Query." ;") ?>
                    <?php if (mysqli_num_rows($getEmp) > 0): ?>
                        <?php while ($emp = mysqli_fetch_assoc($getEmp)):
                            ...
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" id="delete_check_box" name="emp_id[]" value="<?= $emp_id ?>">
