...
//restore 
if (isset($_GET['restore']) && isset($_GET['inward_id'])){
    escapePOST($_GET);
    foreach ($_GET['inward_id'] as $dir){
    //     $delete = mysqli_query($connect, "DELETE FROM inward WHERE inward_id='{$dir}'");
    // }
     $delete = mysqli_query($connect, "UPDATE inward SET inward_status='1' WHERE inward_id='{$dir}'");
    }
        if($delete){
    	header("Location: oneday_add?show_inward=true&action=Success&action_msg=आवक पुनर्संचयित केले..!");
		exit();
        }else{
...
<div class="page-wrapper">
	<div class="page-content">
		<h6 class="mb-0 text-uppercase">आवक</h6>
			...
			<div class="card-body">
				 ...
				 <div class="table-responsive">
					<table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
			                            ...
			                            <div class="dropdown">
			                            	...
			                            	<ul class="dropdown-menu" aria-labelledby="category-action" style="margin: 0px; position: absolute; inset: 0px auto auto 0px; transform: translate(0px, 35px);">
			                            		<li>
			                            			...
			                            			<li>
			                            			<a title="आवक हटवा" class="multi-action-btn dropdown-item text-danger" data-multi-action="delete" data-multi-action-page="" href="?delete=true" onclick="return confirm('आवक हटवा..?');">
			                            				<i class="btn-round bx bx-trash me-2"></i>आवक हटवा 
			                            			</a>
			                            		</li>
			                            	</ul>
						...
						<tbody>
                                        <?php $getBrand = mysqli_query($connect, "select * from inward WHERE inward_status='0' ORDER BY inward_id DESC") ?>
                                        <?php if (mysqli_num_rows($getBrand)>0): ?>
                                            ...
                                            <tr>
                                            <td class="form-group">
                                                <input type="checkbox" class="multi-check-item" name="inward_id[]" value="<?= $inward_id ?>">
                                            ...
                                            <td><a class="text-decoration-none" href="?show_inward=true&inward_id=<?= $inward_id;?>"><?= $inward?></a></td>
                                            <td>₹ <?= $inward_rs?>/-</td>
                                        </tr>
                                            <?php endwhile ?>
                                        <?php endif ?>
            						</tbody>
            						 <?php $gettotal=mysqli_query($connect,"SELECT SUM(inward_rs) as tot FROM inward where inward_status='0'");
