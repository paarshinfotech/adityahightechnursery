...
if (isset($_GET['restore']) && isset($_GET['car_cat_id'])){
    escapePOST($_GET);
    ...
    // }
     $delete = mysqli_query($connect, "UPDATE car_rental_category SET car_cat_status='1' WHERE car_cat_id='{$dir}'");
    }
        if($delete){
    	header("Location: carrental_category_list?action=Success&action_msg=गाडी भाडे श्रेणी पुनर्संचयित कले..!");
		exit();
        }else{
...
if (isset($_GET['delete']) && isset($_GET['car_cat_id'])){
    ...
    //del profile and driver 
    foreach ($_GET['car_cat_id'] as $dir){
    $delete = mysqli_query($connect, "DELETE FROM car_rental_category WHERE car_cat_id='{$dir}'");
     }
        if($delete){
    	header("Location: carrental_category_list?action=Success&action_msg=गाडी भाडे श्रेणी् हटवले..!");
		exit();
        }else{
        header('Location: carrental_category_list?action=Success&action_msg=काहीतरी चूक झाली');
      	exit();
...
<div class="page-wrapper">
	<div class="page-content">
		<h6 class="mb-0 text-uppercase">गाडी भाडे श्रेणी </h6>
            <a class="btn btn-sm btn-success float-end" href="car_rental_category_add" style="margin-top:-25px;">
			...
			<div class="card-body">
				 ...
				 <div class="table-responsive">
					<table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
			                            ...
			                            <div class="dropdown">
			                            	...
			                            	<ul class="dropdown-menu" aria-labelledby="category-action">
			                            		...
			                            		<li>
			                            			...
			                            			<li>
			                            			<a title="गाडी भाडे श्रेणी हटवा" class="multi-action-btn dropdown-item text-danger" data-multi-action="delete" data-multi-action-page="" href="?delete=true" onclick="return confirm('गाडी भाडे श्रेणी हटवा..?');">
						...
						<tbody>
                            <?php $getBrand = mysqli_query($connect, "SELECT * FROM car_rental_category WHERE car_cat_status='0' ORDER BY car_cat_id DESC") ?>
                            <?php if (mysqli_num_rows($getBrand)>0): ?>
                                <?php while ($brand = mysqli_fetch_assoc($getBrand)): 
                                extract($brand);
                                
                                $getdep=mysqli_query($connect,"SELECT SUM(deposit_rs) as totdeposit_rs FROM car_rental WHERE car_cat_id='{$car_cat_id}' and car_status='0'");
                                        $totdep=mysqli_fetch_assoc($getdep);
                                ?>
                                <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" name="car_cat_id[]" value="<?= $car_cat_id ?>">
                                </td>
                                <td>
                                ...
                                <td>
                                    <a class="text-decoration-none" href="carrental_category_add?car_cat_id=<?= $car_cat_id;?>">
                                        <?= $cat_name; ?>
                                    </a>
                                </td>
                                <td>₹ <?= $totdep['totdeposit_rs'] ?>/-</td>
                            </tr>
                                <?php endwhile ?>
                            <?php endif ?>
						</tbody>
