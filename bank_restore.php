...
<?php
Aditya::subtitle('बँक व्यवहार खर्च एमजीबी यादी');
$nav_tabs = array(
	'Inward' => false,
	'expense' => false,
	'bank' => false,
...
//restore 
if (isset($_GET['restore']) && isset($_GET['bank_id'])){
    escapePOST($_GET);
   
    //del profile and driver 
    foreach ($_GET['bank_id'] as $dir){
     $delete = mysqli_query($connect, "UPDATE bank_trans SET bank_status='1' WHERE inward_id='{$dir}'");
    }
        if($delete){
    	header("Location: oneday_add?show_bank=true&action=Success&action_msg=बँक व्यवहार खर्च एमजीबी पुनर्संचयित  केले..!");
		exit();
        }else{
...
if (isset($_GET['delete']) && isset($_GET['bank_id'])){
    ...
    //del profile and driver 
    foreach ($_GET['bank_id'] as $dir){
    $delete = mysqli_query($connect, "DELETE FROM bank_trans WHERE inward_id='{$dir}'");
     }
...
<div class="page-wrapper">
	<div class="page-content">
		<h6 class="mb-0 text-uppercase">बँक व्यवहार खर्च एमजीबी</h6>
			...
			<div class="card-body">
				 ...
				 <div class="table-responsive">
					<table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
			                            ...
			                            <div class="dropdown">
			                            	...
			                            	<ul class="dropdown-menu" aria-labelledby="category-action" style="margin: 0px; position: absolute; inset: 0px auto auto 0px; transform: translate(0px, 35px);">
			                            		<li>
			                            			...
			                            			<li>
			                            			<a title="बँक व्यवहार खर्च एमजीबी हटवा" class="multi-action-btn dropdown-item text-danger" data-multi-action="delete" data-multi-action-page="" href="?delete=true" onclick="return confirm('बँक व्यवहार खर्च एमजीबी हटवा..?');">
			                            				<i class="btn-round bx bx-trash me-2"></i>बँक व्यवहार खर्च एमजीबी हटवा 
			                            			</a>
			                            		</li>
			                            	</ul>
						...
						<tbody>
                                <?php $getBrand = mysqli_query($connect, "select * from bank_trans WHERE bank_status='0' ORDER BY inward_id DESC") ?>
                                <?php if (mysqli_num_rows($getBrand)>0): ?>
                                    ...
                                    <tr>
                                    <td class="form-group">
                                        <input type="checkbox" class="multi-check-item" name="bank_id[]" value="<?= $inward_id ?>">
                                    ...
                                    <td><a class="text-decoration-none" href="?show_bank=true&bank_id=<?= $inward_id;?>"><?= $inward?></a></td>
                                    <td>₹ <?= $inward_rs?>/-</td>
                                </tr>
                                    <?php endwhile ?>
                                <?php endif ?>
    						</tbody>
    						 <?php $gettotal=mysqli_query($connect,"SELECT SUM(inward_rs) as totbank FROM bank_trans WHERE bank_status='0'");
                                        $totalbaktrans=mysqli_fetch_assoc($gettotal);
                                        ?>
                                            <tr>
                                                <th colspan="3" class="text-center fs-6">एकूण</th>
                                                <th class="text-nowrap">₹ <?= $totalbaktrans['totbank'] ?>/-</th>
