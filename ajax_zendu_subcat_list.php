...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    <?php if (true) { ?>
        ...
        $sql = "SELECT COUNT(*) AS total_rows from marigold_category c,marigold_subcategory s, plant_type p where c.cat_id=s.cat_id and sub_cat_status='1' and p.pt_id=s.pt_id  " . $SQL_Filter_Query . " ;";
        ...
        if (($totalRows < 1)) {
            ...
            <table border="1" id="example2" class="table table-striped table-bordered table-hover multicheck-container">
                            ...
                            <div class="d-inline-flex align-items-center select-all">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="category-action">
                                        <li>
                                            <a title="झेंडू उपवर्ग श्रेणी हटवा" class="multi-action-btn dropdown-item text-danger"
                                                data-multi-action="delete" data-multi-action-page="" href="#"
                                                onclick="performDelete('ZENDUSUBCAT', 1, '#delete_check_box', ajaxZanduSubCategoryData)">
                ...
                <tbody>
                    <?php $getBrand = mysqli_query($connect, "SELECT * FROM marigold_category c JOIN marigold_subcategory s ON c.cat_id = s.cat_id JOIN plant_type p ON p.pt_id = s.pt_id WHERE s.sub_cat_status = '1' " . $SQL_Filter_Query . " ORDER BY s.subcat_id DESC " . $SQL_ROW_Query . " ; ") ?>
                    <?php if (mysqli_num_rows($getBrand) > 0): ?>
                        <?php while ($brand = mysqli_fetch_assoc($getBrand)):
                            ...
                            <tr>
                                <td class="form-group">
                                    <input type="checkbox" class="multi-check-item" id="delete_check_box" name="subcat_id[]"
                                        value="<?= $subcat_id ?>">
                                ...
                                <td>
                                    <?= date('d M Y', strtotime($subcat_date)) ?>
                                </td>
